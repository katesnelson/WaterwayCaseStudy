...
setwd('C:/Users/ksnelson/Dropbox/Kate_Paul/paul_simulations/2014_replications_01')
...
colnames(d_sys)<-c("Sim1","Sim2","Sim3","Sim4","Sim5","Sim6","Sim7","Sim8","Sim9","Sim10","Sim11","Sim12","Sim13","Sim14","Sim15","Sim16","Sim17","Sim18","Sim19","Sim20",
                   "Sim21","Sim22","Sim23","Sim24","Sim25","Sim26","Sim27","Sim28","Sim29","Sim30","Sim31","Sim32","Sim33","Sim34","Sim35","Sim36","Sim37","Sim38","Sim39","Sim40",
                   "AIS1","AIS2","AIS3","AIS4","AIS5","AIS6","AIS7","AIS8","AIS9","AIS10","AIS11","AIS12","AIS13","AIS14","AIS15","AIS16","AIS17","AIS18","AIS19","AIS20",
                   "AIS21","AIS22","AIS23","AIS24","AIS25","AIS26","AIS27","AIS28","AIS29","AIS30","AIS31","AIS32","AIS33","AIS34","AIS35","AIS36","AIS37","AIS38","AIS39","AIS40")
...
mydf_m<-melt(d_sys[2:485,]) #avoid the 0.0 hr transit time for first record in sim 22
...
ggplot(mydf_m,aes(x = value)) + stat_ecdf(geom="step",aes(colour = type, linetype=type), size=1) + xlim(0,10) #use type instread of number for colour when plotting all 40 reps
...
ggplot(mydf_m,aes(x = value,colour = type,linetype=type)) + geom_density(aes(), size=1, adjust=1) + xlim(0,10) #use type for colour when plotting all 40 reps
...
ggplot(mydf_m,aes(x = value,colour = type,fill=type)) + 
  geom_density(aes(), size=1, adjust=1, alpha=0.5) + 
  xlim(0,15) + theme_bw() #use type for colour when plotting all 40 reps


spear_r<-cor(as.numeric(mydf_m[mydf_m$number==1 & mydf_m$type %in% "AIS","value"]),as.numeric(mydf_m[mydf_m$number==1 & mydf_m$type %in% "Sim","value"]), method="spearman")
ks<-ks.test(as.numeric(mydf_m[mydf_m$number==1 & mydf_m$type %in% "AIS","value"]),as.numeric(mydf_m[mydf_m$number==1 & mydf_m$type %in% "Sim","value"]), alternative = "two.sided") 

plot(as.numeric(mydf_m[mydf_m$number==1 & mydf_m$type %in% "AIS","value"]), (as.numeric(mydf_m[mydf_m$number==1 & mydf_m$type %in% "AIS","value"]) - as.numeric(mydf_m[mydf_m$number==1 & mydf_m$type %in% "Sim","value"])), xlab="AIS TT", ylab="Residuals (AIS-Sim)", cex=0.5, col="red")
plot(as.numeric(mydf_m[mydf_m$number==1 & mydf_m$type %in% "AIS","value"]), as.numeric(mydf_m[mydf_m$number==1 & mydf_m$type %in% "Sim","value"]), xlab="AIS TT", ylab="SIM TT", cex=0.5, col="red")

hist(as.numeric(mydf_m[mydf_m$number==1 & mydf_m$type %in% "AIS","value"]) - as.numeric(mydf_m[mydf_m$number==1 & mydf_m$type %in% "Sim","value"]), main="Residuals",xlab="value of AIS TT minus Sim TT")

#ensemble stats
spear_r<-cor(as.numeric(mydf_m[mydf_m$type %in% "AIS","value"]),as.numeric(mydf_m[mydf_m$type %in% "Sim","value"]), method="spearman")
ks<-ks.test(as.numeric(mydf_m[mydf_m$type %in% "AIS","value"]),as.numeric(mydf_m[mydf_m$type %in% "Sim","value"]), alternative = "two.sided") 
NSE<-1-(sum((as.numeric(mydf_m[mydf_m$type %in% "Sim","value"])-as.numeric(mydf_m[mydf_m$type %in% "AIS","value"]))^2)/sum((as.numeric(mydf_m[mydf_m$type %in% "AIS","value"])-mean(as.numeric(mydf_m[mydf_m$type %in% "AIS","value"])))^2))
